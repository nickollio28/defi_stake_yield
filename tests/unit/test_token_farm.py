...
from scripts.helpful_scripts import LOCAL_BLOCKCHAIN_ENVIRONMENTS, get_account, get_contract, INITIAL_VALUE, DECIMALS
import pytest
...
def test_set_price_feed_contract():
    ...
    tx = token_farm.setPriceFeedContract(dapp_token.address, price_feed_address, {"from": account})
    tx.wait(1)
    # Assert
    assert token_farm.tokenPriceFeedMapping(dapp_token.address) == price_feed_address
    with pytest.raises(exceptions.VirtualMachineError):
        tx = token_farm.setPriceFeedContract(dapp_token.address, price_feed_address, {"from": non_owner})
        tx.wait(1)
...
def test_issue_tokens(amount_staked):
    ...
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip("Only for local testing")
    account = get_account()
    token_farm, dapp_token = test_stake_tokens(amount_staked)   # must first stake tokens in order to issue them
    ...
    # so we should get 2000 dapp tokens in rewards
    # since the price of ETH from deploying MockV3Aggregator is $2000
...
def test_add_allowed_tokens():
    ...
    token_farm, dapp_token = deploy_token_farm_and_dapp_token()
    # Act
    tx = token_farm.addAllowedTokens(dapp_token.address, {"from": account})
    tx.wait(1)
    # Assert
